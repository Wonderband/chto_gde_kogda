...
function buildReadScript(gameContext) {
  ...
  const meta = char
    ? {
        name: isRu ? char.ruName : char.ukName,
        city: char.city,
        desc: isRu ? char.ruDesc : char.ukDesc,
      }
  ...
  if (q.round_type === "blitz") {
    ...
    const posLabel = blitzPositionLabel(pos, lang);
    const lines = [];
...
function buildEvaluationInstructions() {
  return [
    ...
    "- no_answer: ПРАВИЛО ЗА ЗАМОВЧУВАННЯМ — у будь-якому СУМНІВНОМУ випадку no_answer: true. Краще попросити повторити, ніж прийняти нісенітницю. no_answer: false ЛИШЕ якщо команда чітко назвала конкретну особу, предмет, поняття або факт — однозначно ідентифікований, навіть якщо хибний (приклади коли false: «Гектор Саламанка», «термометр», «залізо літій натрій», «не знаю але може Флінн»). no_answer: true в УСІХ інших випадках: (1) порожній текст або < 3 символів; (2) більшість слів — вигадані або незрозумілі склади («муви кукс», «Гзен одрбогур»); (3) команда зізналась що не знає або відмовилась («не знаю», «забула», «пропускаємо», «важко сказати»); (4) фонова стороння розмова («Мама, можна піти», «зараз виключу молоко»); (5) текст беззмістовний або не містить жодного конкретного імені / поняття / факту що можна ідентифікувати як спробу відповісти. Якщо Retry attempt >= 1 — ЗАВЖДИ no_answer: false.",
  ].join("\n");
}
...
function buildEvaluationInput(gameContext) {
  ...
  return [
    ...
    gameContext.retry_attempt !== undefined
      ? `Retry attempt: ${gameContext.retry_attempt}`
      : "",
    gameContext.score
...
function evaluationSchema() {
  return {
    ...
    schema: {
      ...
      required: ["reasoning", "correct", "correct_answer_reveal", "no_answer"],
      properties: {
...
async function callStructuredEvaluation(gameContext) {
  ...
  if (parsed && typeof parsed === "object") {
    ...
    console.log("[Evaluator][OUTPUT]", {
      ...
      correct_answer_reveal: parsed.correct_answer_reveal,
    });
