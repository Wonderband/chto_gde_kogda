...
function attentionLineForQuestion(gameContext) {
  ...
  if (pos === 1) {
    return isRu
      ? "Сектор Блиц! Три вопроса. Двадцать секунд на каждый. Внимание! Первый вопрос!"
      : "Сектор Бліц! Три питання. Двадцять секунд на кожне. Увага! Перше питання!";
  }
...
ТЕКУЩАЯ ФАЗА: СТАРТОВАЯ РЕПЛИКА — РАУНД ${round}.

ЗАДАЧА — строго две короткие фразы:
1. Объяви номер раунда: «Раунд ${round}!» (или краткий синоним).
2. Задай ОДИН лёгкий вопрос игрокам в духе: «${seed}»
После второй фразы сразу замолчи и жди.

ЖЁСТКИЕ ЗАПРЕТЫ:
- не говори больше двух фраз суммарно,
- не упоминай персонажей Breaking Bad, сектор или будущий вопрос,
- каждая фраза — максимум 8 слов,
- не продолжай монолог после вопроса — жди ответа игроков.

Текущий счёт: ${scoreExperts} : ${scoreViewers}.
